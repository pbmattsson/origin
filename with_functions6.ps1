...
function Get-ModemStatus {
	param($apa)
	get-wmiobject "Win32_POTSModem" -computername $apa|Where-Object  {$_.Status -ne "OK"} |
		Select-Object SystemName,Name,AttachedTo,Status,StatusInfo, @{Name="DriverDate";Expression={$_.DriverDate.Substring(0,8)
		}}|Export-csv c:\$args[0].csv -Append
		Write-Host c:\$args[1].csv created
		}
...
$script:activecomputers | foreach-object {Get-Modemstatus -apa ($_)}
