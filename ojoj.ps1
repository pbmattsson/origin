...
function Get-Uptime {
       ...
       #test-connection -count 1 -ComputerName $computername -TimeToLive 15 -asJob -ThrottleLimit 100 |
		Get-WMIObject -class Win32_OperatingSystem -ComputerName $computername -TimeToLive 15 -AsJob -ThrottleLimit 100 |
...
$online |sort
	$Bootup = $systems.LastBootUpTime
            ...
            $m = $uptime.Minutes
            # Display uptime
            $Uptime = [string]::format("{0} Days {1} Hours {2} Minutes", $d,$h,$m)
$Uptime = Get-Uptime -computername $online
...
ForEach ($computername in $online) 
	...
	if ($pingresult.status –eq “Success”)
	#if ($ping.Send($item))
		{
			...
			Write-Host $computername "pinged OK" "System uptime = " $Uptime -ForegroundColor Green
			Write-Host
